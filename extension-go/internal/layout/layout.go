// Package layout owns the S3 key layout: where a log object goes and which
// partition it belongs to. The extension writes these keys and the compactor
// rewrites files within them, so both must derive paths from one definition.
package layout
